...
`request_url`::
	...
	The full URL of the request for which a credential is needed, as
	Git actually tried it (including, for example, a path and query
	string that are otherwise dropped from the `path` attribute for
	HTTP(S) URLs). Unlike the `url` attribute, this value is not parsed
	into its constituent parts; it is passed through to helpers verbatim
	so that they have the complete request context. This attribute is
	'one-way' from Git to pass additional information to credential
	helpers, and is not used to select or match credentials.
