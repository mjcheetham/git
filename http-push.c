...
static struct curl_slist *add_dav_token_headers(struct curl_slist *headers,
...
static int refresh_lock(struct remote_lock *lock)
{
	...
	slot = get_active_slot(0);
	slot->headers = add_dav_token_headers(slot->headers, lock,
